...
P.S.: When you find yourself copy-pasting those `_PENDING`, `_SUCCESS` and `_ERROR` event names from one action creator to another then take a look at `asynchronousActionEventNaming` setting described in the [Additional `react-isomorphic-render.js` settings](https://github.com/halt-hammerzeit/react-isomorphic-render#additional-react-isomorphic-renderjs-settings) section of this document: it lets a developer just supply a "base" `event` name and then it generates the three lifecycle event names from that "base" `even` reducing boilerplate significantly.
