...
import { getFromContext, setInContext, clearInContext, clearAllInContextExcept } from '../../context.js'
...
function setUpAndRender(settings, options, { redirectException, stash }) {
	...
	const onSuccess = (result) => {
		...
		if (isServerSideLoaded()) {
			...
			const eventPayload = getFromContext('InitialPage/FakeResolveMatchEventPayload')
			clearInContext('InitialPage/FakeResolveMatchEventPayload')
			store.dispatch({
				type: _RESOLVE_MATCH,
