...
import { redirect, RESOLVE_MATCH, _RESOLVE_MATCH, pushLocation, RedirectException } from '../../router/index.js'
...
export default function setUpAndRender(settings, options) {
	...
	const shouldShowInitialLoad = Boolean(InitialLoadComponent) && !isServerSidePreloaded()
	...
	const promise = clientSideRender({
	...
	return onFoundRouterReady(() => promise.then(onSuccess, onError))
}